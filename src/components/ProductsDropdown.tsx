...
const ProductsDropdown = ({ onPageChange }: ProductsDropdownProps) => {
  ...
  const handleClick = (href: string, category: string, subcategory?: string, productId?: string, e?: React.MouseEvent) => {
    ...
    console.log(`Navigating to: ${href}, category: ${category}, productId: ${productId || 'none'}`);
          ...
          {filteredMenuItems.map((item) => (
            ...
            >
              ...
              <button 
                onClick={(e) => {
                  ...
                  if (isMobile) {
                    ...
                    console.log(`Desktop click: Navigating to ${item.directLink.href}`);
                    handleClick(
                      item.directLink.href, 
                      item.directLink.category, 
                      undefined,
              ...
              {!isMobile && item.items && item.items.length > 0 && activeSubmenu === item.translationKey && (
                <div 
                  className="absolute top-0 left-full ml-0 w-72 bg-white rounded-lg shadow-lg border border-gray-100 py-2 z-50"
                ...
                >
                  {item.items.map((subItem) => (
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        handleClick(subItem.href, subItem.category, subItem.subcategory, subItem.productId, e);
                      }}
                      className="block w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#96cc39]"
                    >
                ...
                <div className="bg-gray-50 py-2">
                  {item.items.map((subItem) => (
                    <button
                      ...
                      className="block w-full text-left px-8 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-[#96cc39]"
                    >
                      {t(subItem.translationKey)}
                    </button>
                  ))}
