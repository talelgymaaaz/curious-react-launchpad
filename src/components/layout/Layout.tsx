...
import { AssistantNotification } from '../ui/assistant-notification';
...
const Layout = ({ children }: LayoutProps) => {
  ...
  // Show notification for product pages or when assistant becomes available
  useEffect(() => {
    if (showAssistant && (location.pathname === '/' || location.pathname.startsWith('/product/'))) {
      const timer = setTimeout(() => {
        setShowNotification(true);
      }, 1000);
  ...
  }, [location.pathname, showAssistant]);
  ...
  return (
    <div className={`min-h-screen ${isIndexPage ? 'bg-transparent' : 'bg-white'}`}>
      ...
      {!isAdminPage && showNotification && (
        <AssistantNotification onClose={handleNotificationClose} />
      )}
    </div>
  );
};
