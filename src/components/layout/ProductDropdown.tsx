...
const ProductDropdown = ({ isOpen, activeCategory, onClose }: ProductDropdownProps) => {
          ...
          <div className="space-y-2">
            {Object.entries(t('surMesure.homme', { returnObjects: true }) as Record<string, string>)
              ...
              .map(([key, value]) => (
                <button
                  key={key}
                  onClick={() => handleCategoryClick('surMesure', `homme-${key}`)}
                  className="block text-gray-600 hover:text-blue-600 transition-colors text-sm w-full text-left"
                >
                  {value}
                </button>
              ))}
          ...
          <div className="space-y-2">
            {Object.entries(t('surMesure.femme', { returnObjects: true }) as Record<string, string>)
              ...
              .map(([key, value]) => (
                <button
                  key={key}
                  onClick={() => handleCategoryClick('surMesure', `femme-${key}`)}
                  className="block text-gray-600 hover:text-blue-600 transition-colors text-sm w-full text-left"
                >
                  {value}
                </button>
      ...
      <div>
        ...
        <div className="space-y-2">
          {Object.entries(t('pretAPorter', { returnObjects: true }) as Record<string, string>).map(([key, value]) => (
            <button
              key={key}
              onClick={() => handleCategoryClick('pretAPorter', key)}
              className="block text-gray-600 hover:text-blue-600 transition-colors text-sm w-full text-left"
            >
              {value}
            </button>
      ...
      <div>
        ...
        <div className="space-y-2">
          {Object.entries(t('accessoires', { returnObjects: true }) as Record<string, string>).map(([key, value]) => (
            <button
              key={key}
              onClick={() => handleCategoryClick('accessoires', key)}
              className="block text-gray-600 hover:text-blue-600 transition-colors text-sm w-full text-left"
            >
              {value}
            </button>
  ...
  return (
    <div className="fixed inset-x-0 z-50 bg-white shadow-lg border-t border-gray-100" style={{ top: 'calc(42px + 120px)' }}>
