...
const ProductModalContent = ({ 
  ...
  const removeImage = (index: number) => {
    ...
    // If it's a blob URL (new uploaded image), clean it up and remove from selectedImages
    if (urlToRemove && urlToRemove.startsWith('blob:')) {
      URL.revokeObjectURL(urlToRemove);
      
      // Find the corresponding file in selectedImages and remove it
      ...
      const existingIndex = existingImages.findIndex(path => 
        `https://draminesaid.com/lucci/${path}` === urlToRemove
      );
      if (existingIndex !== -1) {
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                <Label className="text-sm font-medium text-gray-700 mb-2 block">Collection/Saison</Label>
                <Input
                  className="border-gray-300 focus:border-black focus:ring-black"
                  placeholder="Ex: Collection Été, Wedding Package, Collection Nouveau"
                  value={formData.collection_product || ''}
                  onChange={(e) => updateFormData('collection_product', e.target.value)}
                />
              </div>
...
const AdminProducts = () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          collection_product: productData.collection_product || '',
        ...
        if (productData.img_product) {
          existingImageUrls.push(`https://draminesaid.com/lucci/${productData.img_product}`);
          existingImagePaths.push(productData.img_product);
        }
        if (productData.img2_product) {
          existingImageUrls.push(`https://draminesaid.com/lucci/${productData.img2_product}`);
          existingImagePaths.push(productData.img2_product);
        }
        if (productData.img3_product) {
          existingImageUrls.push(`https://draminesaid.com/lucci/${productData.img3_product}`);
          existingImagePaths.push(productData.img3_product);
        }
        if (productData.img4_product) {
          existingImageUrls.push(`https://draminesaid.com/lucci/${productData.img4_product}`);
          existingImagePaths.push(productData.img4_product);
        }
        
        setImagePreviewUrls(existingImageUrls);
        setExistingImages(existingImagePaths);
        setSelectedImages([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!processedData.collection_product) processedData.collection_product = '';
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get the original existing images from the product
      const originalExistingImages = [
        editingProduct.img_product,
        editingProduct.img2_product,
        editingProduct.img3_product,
        editingProduct.img4_product
      ];

      // Determine which existing images to keep
      const keptExistingImages = existingImages.filter(imgPath => imgPath);
      
      // Determine which existing images to delete
      const imagesToDelete = [];
      originalExistingImages.forEach((originalImage, index) => {
        if (originalImage && !keptExistingImages.includes(originalImage)) {
          imagesToDelete.push(`img${index === 0 ? '' : index + 1}_product`);
        }
      });

      // Send existing images to delete
      if (imagesToDelete.length > 0) {
        formDataToSend.append('images_to_delete', JSON.stringify(imagesToDelete));
      }

      // Append new images to fill available slots
      let imageSlot = 1;
      
      // First, account for kept existing images
      keptExistingImages.forEach(() => {
        imageSlot++;
      });
      
      // Then add new images to remaining slots
      selectedImages.forEach((image) => {
        if (imageSlot <= 4) {
          formDataToSend.append(`image${imageSlot}`, image);
          imageSlot++;
        }
      });

      const response = await fetch(`https://draminesaid.com/lucci/api/update_product_with_images.php?id=${editingProduct.id_product}`, {
        method: 'POST',
        body: formDataToSend,
      });
